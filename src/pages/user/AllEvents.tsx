...
export function Component() {
    ...
    const [selectedEvent, setSelectedEvent] = useState<IDocumentEvent | IMeetingEvent | IActivityEvent | null>(null);

    const showEventModal = (record: IDocumentEvent | IMeetingEvent | IActivityEvent) => {
    ...
    const columns: ColumnsType<(IDocumentEvent | IMeetingEvent | IActivityEvent)> = [
        ...
        },
        {
          ...
          onFilter: (value, record) => record.creationTeamName.indexOf(value as string) === 0,
        },
        {
          title: 'Creator',
        ...
        },
        {
            ...
            fixed: 'right',
            width: 100,
            render: ( record) => record.id ? <a onClick={() => showEventModal(record)}>View</a> : null,
          },
    ...
    return(
      ...
      >
        {selectedEvent && (
          <div>
            ...
            <p className="text-base">Description: {selectedEvent.description}</p>
        ...
        {selectedEvent?.type === 'document' && (
            ...
            {/* Display Document Event specific information */}
                <p className="text-base">Link: {(selectedEvent as IDocumentEvent).link}</p>
                <p className="text-base">Deadline Date: {(selectedEvent as IDocumentEvent).deadlineDate?.toLocaleString()}</p>
            </div>
        )}
      ...
      {selectedEvent?.type === 'meeting' && (
        <div>
          ...
          <p className="text-base">Virtual: {(selectedEvent as IMeetingEvent).virtual ? 'Yes' : 'No'}</p>
          <p className="text-base">Location: {(selectedEvent as IMeetingEvent).location}</p>
          <p className="text-base">Start Time: {(selectedEvent as IMeetingEvent).startTime.toLocaleString()}</p>
          <p className="text-base">End Time: {(selectedEvent as IMeetingEvent).endTime.toLocaleString()}</p>
          <p className="text-base">Note Link: {(selectedEvent as IMeetingEvent).noteLink}</p>
          <p className="text-base">Agenda Link: {(selectedEvent as IMeetingEvent).agendaLink}</p>
          <p className="text-base">Meeting Type: {(selectedEvent as IMeetingEvent).meetingType}</p>
        </div>
      )}
      ...
      {selectedEvent?.type === 'activity' && (
        <div>
          ...
          <p className="text-base">Virtual: {(selectedEvent as IActivityEvent).virtual ? 'Yes' : 'No'}</p>
          <p className="text-base">Location: {(selectedEvent as IActivityEvent).location}</p>
          <p className="text-base">Start Time: {(selectedEvent as IActivityEvent).startTime.toLocaleString()}</p>
          <p className="text-base">End Time: {(selectedEvent as IActivityEvent).endTime.toLocaleString()}</p>
        </div>
      )}
      </Modal>
        </div>
