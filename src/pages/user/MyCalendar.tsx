...
import { IDocumentEvent, IMeetingEvent, IActivityEvent, ITicket, ICalendarItem } from "../../types";
import { useGetEmployeeTeamsQuery, useGetTicketsQuery } from "../../redux/api/apiSlice";
import { mapEventDataToEvent, mapDataToTickets, mapEventToCalendarItem, mapTicketToCalendarItem } from "../../utils/functions";
...
export function Component() {
    ...
    useEffect(() => {
        if(eventList && ticketList){
            const mappedEventItems = eventList.map((event)=>mapEventToCalendarItem(event));
            const mappedTicketItems = ticketList.map((ticket)=> mapTicketToCalendarItem(ticket));
