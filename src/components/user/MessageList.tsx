...
const MessageList: React.FC<MessageListProps> = ({ logs, currentUserId }) => {
    ...
    const sortedLogs = [...logs].sort((a, b) => a.ticketLogCreationdate.getTime() - b.ticketLogCreationdate.getTime());
  ...
  return (
    <div className="message-list relative h-full">
      {allLogs.map((log) => (
        ...
        <div className={`flex items-center space-x-2 ${log.ticketLogCreator === currentUserId ? 'justify-end' : 'justify-start'}`}>
          <div className={`p-3 max-w-md w-80 rounded-lg ${log.ticketLogCreator === currentUserId ? 'bg-white' : 'bg-blue-100'}`}>
            <p className="text-xs">{log.ticketLogContent}</p>
            <div className="text-sm mt-3">
                {log.ticketLogCreator === currentUserId ? 'You' : `${log.ticketLogCreatorName}`}
                <span>   </span>
