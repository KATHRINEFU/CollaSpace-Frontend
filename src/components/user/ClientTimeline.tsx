...
const ClientTimeline: React.FC<ClientTimelineProps> = ({currentDepartmentId, currentStatus}) => {
    const timelineData = clientStatusByDepartment.map((item, index) => {
        ...
        return {
          color: color, 
          children: [
            <p className={isCurrentDepartment ? 'text-2xs font-bold':'text-sm'}>{`${item.department}`}</p>,
            ...item.processes.map((process, index) => (
                <div className='flex gap-3'>
                     <p className={isCurrentDepartment ? 'text-2xs':'text-xs'} key={index}>
                        {process}
                    </p>
