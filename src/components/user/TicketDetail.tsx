...
const TicketDetail: React.FC<TicketDetailProps> = ({
  ...
  ticketForm.setFieldsValue({
    selectedTicket,
    dueDate: moment(initialValue.dueDate),
  });
  ...
  const isAllowChangeStatus =
    isEditting && (isAssignee || isCreator || isSupervisor);
      ...
      for (const viewer of viewers) {
        try {
          const response = await axios.get(
            `/api/employee/${viewer.employeeId}`,
          );
  ...
  useEffect(() => {
    const fetchEmployeeProfiles = async () => {
      ...
      for (const supervisor of supervisors) {
        try {
          const response = await axios.get(
            `/api/employee/${supervisor.employeeId}`,
          );
          profiles.push(response.data);
        } catch (error) {
          console.error("Error fetching employee info: ", error);
                  ...
                  <div>
                    <Select
                      ...
                      options={[
                        ...
                        {
                          value: TicketStatus.UNDER_REVIEW,
                          label: "UNDER REVIEW",
                        },
              ...
              <div className="flex gap-3 items-center">
                ...
                >
                  {viewerProfiles.map((profile, index) => (
                    ...
                    >
                      <Avatar
                        src={profile ? profile.employeeProfileUrl : ""}
                        style={{ backgroundColor: "#bae0ff" }}
                      >
                        {profile
                          ? `${profile.employeeFirstname.charAt(
                              0,
                            )}${profile.employeeLastname.charAt(0)}`
                          : "N/A"}
                      </Avatar>
                    </Tooltip>
                  ))}
