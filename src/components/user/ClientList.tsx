...
const ClientList: React.FC<ClientListProps> = ({ accountList, filterOptions }) => {
    ...
    const filteredAccounts = accountList.filter((account) => {
        const statusFilterMatch =
          ...
          filterOptions.status.map(type => type.toLowerCase()).includes(account.accountCurrentStatus);
    
        const typeFilterMatch =
          filterOptions.type.length === 0 ||
          filterOptions.type.map(type => type.toLowerCase()).includes(account.accountType); 
